...
var incrmvariance = require( './../../dist' );
...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof incrmvariance, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function throws an error if not provided a number as the mean value', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			incrmvariance( 3, value );
		};
	}
});

tape( 'the function returns an accumulator function', function test( t ) {
	t.equal( typeof incrmvariance( 3 ), 'function', 'returns a function' );
	t.end();
});

tape( 'the function returns an accumulator function (known mean)', function test( t ) {
	t.equal( typeof incrmvariance( 3, 3.0 ), 'function', 'returns a function' );
	t.end();
});
...
tape( 'if not provided an input value, the accumulator function returns the current unbiased sample variance', function test( t ) {
	...
	t.equal( acc(), 0.5, 'returns current unbiased sample variance' );
...
tape( 'if not provided an input value, the accumulator function returns the current unbiased sample variance (known mean)', function test( t ) {
	...
	data = [ 2.0, 3.0, 10.0 ];
	acc = incrmvariance( 3, 5.0 );
	for ( i = 0; i < data.length-1; i++ ) {
		acc( data[ i ] );
	}
	t.equal( acc(), 6.5, 'returns current unbiased sample variance' );
...
tape( 'if the window size is `1` and the mean is known, the accumulator function may not always return `0`', function test( t ) {
	...
	acc = incrmvariance( 1, 500.0 ); // mean is outside the range of simulated values so the variance should never be zero
	for ( i = 0; i < 100; i++ ) {
		s2 = acc( randu() * 100.0 );
